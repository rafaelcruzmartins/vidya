...
correções dos materiais complementares aplicadas.
...
4. Copie **todo o conteúdo** de `vidya-main` para dentro de `addons/vidya`.
   O arquivo `config.yaml` precisa ficar na raiz dessa pasta, ao lado do
   `Dockerfile`.
...
1. Substitua os arquivos em `addons/vidya` pela versão nova.
2. Aumente o campo `version` no `config.yaml` (por exemplo, para `"1.0.2"`).
